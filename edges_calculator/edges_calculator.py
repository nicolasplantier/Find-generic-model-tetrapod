"""
The aim of the file if to create the edges of the image of the tetrapods in order to prepare the data for the watershed algorithm. 
"""
...
def run(): 
    ...
    path_patches_classified = f.os.path.join(parent_path, f"ajaccio_patches_classified")
    list_patches_names = (f.os.listdir(path_patches_classified))
    list_patches_names.remove(".DS_Store")

    for filename in list_patches_names:
        file_number = filename[25:-4]
        las = f.laspy.read(f.os.path.join(parent_path, f"ajaccio_patches_classified", filename)) 
        n = len(las.x)
        ...
        print(f"The image for file number {file_number} is saved")
